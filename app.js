/* RaceBox Web - app.js (Final Fix v4 – Anti Auto Start + Anti Drift)
   - Start ONLY when real movement (speed >= 1 km/h for ≥400ms AND moved ≥2 meter)
   - Ghost-speed filter strong (speed < 2.0 km/h = 0)
   - RUN never continues when speed = 0 (anti GPS drift)
   - ARM logic fixed
*/
...
(() => {
  ...
  function onPos(p) {
    ...
    // GHOST SPEED FILTER (lebih keras)
    ...
    // =======================================
    // RUNNING MODE (ANTI DRIFT)
    // =======================================

    // Stop distance jika motor benar-benar berhenti
    if (cur.speed === 0) {
      return; // anti drift
    }
